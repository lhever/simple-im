 ...
 * </p>
 *
 * @author lihong10 2020/11/4 21:47
 * @version v1.0
 * @modificationHistory=========================逻辑或功能性重大变更记录
 * @modify by user: {修改人} 2020/11/4 21:47
 * @modify by reason:{方法名}:{原因}
 */
public final class ThreadLocalRandom extends Random {

    private static final Logger logger = LogFactory.getLogger(ThreadLocalRandom.class);
    ...
    static {
        initialSeedUniquifier = SystemPropertyUtils.getLong("com.hikvision.core.initialSeedUniquifier", 0);
        if (initialSeedUniquifier == 0) {
            boolean secureRandom = SystemPropertyUtils.getBoolean("java.util.secureRandomSeed", false);
            if (secureRandom) {
                ...
                if (current == 0 && logger.isDebugEnabled()) {
                    if (seedGeneratorEndTime != 0) {
                        logger.debug(String.format(
                                "-Dcom.hikvision.core.initialSeedUniquifier: 0x%016x (took %d ms)",
                                actualCurrent,
                                TimeUnit.NANOSECONDS.toMillis(seedGeneratorEndTime - seedGeneratorStartTime)));
                    } else {
                        logger.debug(String.format("-Dcom.hikvision.core.initialSeedUniquifier: 0x%016x", actualCurrent));
                    }
                }
